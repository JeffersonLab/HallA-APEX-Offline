...
void BPM_calibration(string arm ="R", string harp_date = "02162019", int debug =0, int ped_run=0){
	//get rid of stat box
  	gStyle->SetOptStat(0);
...
/////////////////////////////////////////////////////////////////////////
	if(ped_run!=0)
     ...
     int counts[8];
     TString name[8] = {"xp","xm","yp","ym"};

///////////// BPM wire loop!
     for(int i =0; i<8;i++){
             if(i<4){H[i] = new TH1F(Form("h%d",i),Form("BPMA %s",name[i].Data()),60000,0,60000);
             c11->cd(i+1); t->Draw(Form("%srb.BPMA.rawcur.%d>>h%d",arm.c_str(),i+1,i));}
             else{
               H[i] = new TH1F(Form("h%d",i),Form("BPMB %s",name[i-4 ].Data( ) ) ,60000,0,60000);
             c12->cd(i-3); t->Draw(Form("%srb.BPMB.rawcur.%d>>h%d",arm.c_str(),i-3,i));}
...
//Setting up the BPM histograms and tree varibles!!
	//For BPMA
	for(int A=1;A<5;A++){T->SetBranchAddress(Form("%srb.BPMA.rawcur.%d",arm.c_str(),A),&peak[A]);
			H[A] = new TH1F(Form("H%d",A),Form("%s BPMA_%d",arm.c_str(),A),10000,0,100000);
			HH[A]= new TH1F(Form("HH%d",A),Form("%s BPMA_%d",arm.c_str(),A),25000,pedestal[A]+200,10000);}
	//For BPMB
	for(int B=1;B<5;B++){T->SetBranchAddress(Form("%srb.BPMB.rawcur.%d",arm.c_str(),B),&peak[B+4]);
			H[B+4] = new TH1F(Form("H%d",B+4),Form("%s BPMB_%d",arm.c_str(),B),10000,0,100000);
			HH[B+4]= new TH1F(Form("HH%d",B+4),Form("%s BPMA_%d",arm.c_str(),B),25000,pedestal[B+4]+200,10000);}
	c1[numofscans]->cd(1);
	...
	TCut current = "1";// Form("hac_bcm_average>=%f",cur[numofscans]);
		...
		for(int m=1;m<9;m++){c1[numofscans]->cd(m);
			...
			//M<5 means BPMA
			if(m<5){T->Draw(Form("%srb.BPMA.rawcur.%d>>H%d",arm.c_str(),m,m),current);
			...
			////////////////////////////////////////////////////////////////////////
			//M>=5 means BPMB
			if(m>=5){T->Draw(Form("%srb.BPMB.rawcur.%d>>H%d",arm.c_str(),m-4,m),current);
				peak[m] = H[m]->GetBinCenter(H[m]->GetMaximumBin());
