...
void APEX_PID_check_R()
{
  ...
  // Initial 'good' electron, pion and other background cuts
  ...
  Double_t e_cut_cal_h = 1.15;
  Double_t e_cut_cal_l = 0.92;
  ...
  c1->SaveAs("plots/R_event_selection.pdf");
  
  
  gSystem->Exec("convert -density 700 -trim plots/R_event_selection.pdf plots/R_event_selection.png");
  ...
  // TH1D* Theta_plot_e =  new TH1D("Theta_plot_e","",100,-2,2);
  ...
  // TH1D* Phi_plot_e =  new TH1D("Phi_plot_e","",100,-2,2);
  ...
  c2->SaveAs("plots/R_track_var_plots.pdf");
   
  
  gSystem->Exec("convert -density 700 -trim plots/R_track_var_plots.pdf plots/R_track_var_plots.png");
  ...
  TCut gen_cut = Form("R.tr.n==1 && R.gold.beta>%f && R.gold.th>%f && R.gold.th <%f && R.gold.ph>%f && R.gold.ph <%f && R.gold.dp>%f && R.gold.dp<%f", beta_cut_l, theta_cut_l, theta_cut_h, phi_cut_l, phi_cut_h, dp_cut_l, dp_cut_h);
 ...
 Double_t e_sh_min = 0.12;
 Double_t e_sh_max = 1.05;
 ...
 Double_t pi_sh_min = 0.07;
 Double_t pi_sh_max = 0.17;
 ...
 // electron cuts based on PS
  ...
  Double_t e_ps_min_1 = 0.10;
  Double_t e_ps_max_1 = 0.87;
  ...
  c3->SaveAs("plots/R_Cherenkov_scan.pdf");
   
  
  gSystem->Exec("convert -density 700 -trim plots/R_Cherenkov_scan.pdf plots/R_Cherenkov_scan.png");
  ...
  cout << "Cal_EP_plot has " << Cal_EP_plot->GetEntries() << " entries" << endl;
 ...
 c4->cd(2);
  ...
  // write out level of lower PS + SH cut

  Double_t PS_2_cut_l = 0.625;

  
  TLine* PS_2_fin_cut_l = new TLine(PS_2_cut_l,0,PS_2_cut_l,Cal1_2_EP_plot_e->GetMaximum());
  PS_2_fin_cut_l->SetLineColor(kMagenta);
  PS_2_fin_cut_l->SetLineWidth(2);



  // write out level of upper PS + SH cut

  Double_t PS_2_cut_h = 1.25;

  
  TLine* PS_2_fin_cut_h = new TLine(PS_2_cut_h,0,PS_2_cut_h,Cal1_2_EP_plot_e->GetMaximum());
  PS_2_fin_cut_h->SetLineColor(kMagenta);
  PS_2_fin_cut_h->SetLineWidth(2);
  ...
  PS_2_fin_cut_l->Draw("same");
  PS_2_fin_cut_h->Draw("same");
  ...
  c4->SaveAs("plots/R_Calorimeter_scan.pdf");
     
  gSystem->Exec("convert -density 700 -trim plots/R_Calorimeter_scan.pdf plots/R_Calorimeter_scan.png");

  gSystem->Exec("pdfunite plots/R_event_selection.pdf plots/R_track_var_plots.pdf plots/R_Cherenkov_scan.pdf plots/R_Calorimeter_scan.pdf plots/R_PID_plots_combined.pdf");
