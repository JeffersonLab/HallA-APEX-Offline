...
Script that performs timing offset calibration for s0 and s2 for the LHRS.
...
void L_s_both_timing(Int_t runno){


  //  TChain* t = Load_more_rootfiles(runno);

  TChain* t = new TChain("T");
  t->Add("/w/work3/home/johnw/Rootfiles/apex_online_14_1_21_tof_tw_4771.root");// TOF + TW
  ...
  Double_t L_x_l[16] = {-1.1,-0.99,-0.88,-0.70,-0.55,-0.40,-0.3,-0.15,-0.01,0.12,0.27,0.40,0.53,0.68,0.82};
  Double_t L_x_h[16] = {-0.95,-0.83,-0.68,-0.55,-0.40,-0.24,-0.12,0.0,0.17,0.29,0.40,0.55,0.69,0.82,0.99,1.04};

  
  
  
  //Initialize S0 Histograms
  TH2F *h1a = new TH2F("h1a","LHRS Left_s0_TDC vs X: Good Events",500,-1.25,1.25,400,2600,3000);
  h1a->GetXaxis()->SetTitle("X_s0 [m]"); h1a->GetXaxis()->CenterTitle();
  h1a->GetYaxis()->SetTitle("TDC Channel"); h1a->GetYaxis()->CenterTitle();
  
  TH2F *h1b = new TH2F("h1b","LHRS Right_s0_TDC vs Track  X: Good Events",500,-1.25,1.25,400,2600,3000);
  ...
  // needed to extract S2 offsets later on
  ...
  for(Int_t i=0;i<nentries;i++){
    ...
    for(Int_t j=0;j<16;j++){
      if(L_tr_n==1 && L_cer_asum_c>2500&&L_s2_t_pads[0]==j&&L_s0_nthit==1&&(L_ps_e+L_sh_e)/L_tr_p[0]/1000>0.7 && L_s2_trx[0]>L_x_l[j] && L_s2_trx[0]<L_x_h[j]){

	// if(i%1000==0){
	//   cout << "trx = " << L_s2_trx[0] << ", j = " << j << ", with lims " << L_x_l[j] << " & "  << L_x_h[j] <<  endl;
	// }


	tof=(L_s2_trpath[0]-L_s0_trpath[0])/elec_v;
        TOF=tof/ctos;
	//        Double_t SUM=(L_s0_lt[0]+L_s0_rt[0])-(L_s2_rt[j]+L_s2_lt[j])+(L/c0/ctos)-(l/c_s2/ctos)+3018.26-2*TOF;
  ...
  c1->Print(Form("plots/L_s0_timing_%i.pdf[",runno));
  c1->Print(Form("plots/L_s0_timing_%i.pdf",runno));
  c2->Print(Form("plots/L_s0_timing_%i.pdf",runno));
  c2->Print(Form("plots/L_s0_timing_%i.pdf]",runno));
  ...
  for(Int_t i=0;i<4;i++){
    ...
    if(i==0){
      c3[i]->Print(Form("plots/L_s2_timing_%i.pdf[",runno));
      c3[i]->Print(Form("plots/L_s2_timing_%i.pdf",runno));
    }
    else{
      c3[i]->Print(Form("plots/L_s2_timing_%i.pdf",runno));
  ...
  for(Int_t i=0;i<8;i++){
	...
	s2_speed[scin_count] = abs((2./temp_speed)/ctos);
      ...
      c4[i]->Print(Form("plots/L_s2_timing_%i.pdf",runno));
  }
  ...
  for(Int_t i=0;i<nentries;i++){
    ...
    Int_t x=evtypebits;
    for(Int_t j=0;j<16;j++){
      if(L_tr_n==1 && L_cer_asum_c>2500&&L_s2_t_pads[0]==j&&L_s0_nthit==1&&(L_ps_e+L_sh_e)/L_tr_p[0]/1000>0.7){

	tof=(L_s2_trpath[0]-L_s0_trpath[0])/elec_v;
	TOF=tof/ctos;
	//	Double_t SUM=(L_s0_lt[0]+L_s0_rt[0])-(L_s2_rt[j]+L_s2_lt[j])+(L/c_speed/ctos)-(l/s2_speed/ctos)+3018.26-2*TOF;
	Double_t SUM=(L_s0_lt[0]+L_s0_rt[0])-(L_s2_rt[j]+L_s2_lt[j])+(L/c_speed/ctos)-(l/s2_speed[j]/ctos)+ ((t_stop-corr_left)+(t_stop-corr_right)) - 2*TOF;
  ...
  for(Int_t i=0;i<8;i++){
    ...
    if(i==7){
      c5[i]->Print(Form("plots/L_s2_timing_%i.pdf",runno));
      c5[i]->Print(Form("plots/L_s2_timing_%i.pdf]",runno));    
    }
    else c5[i]->Print(Form("plots/L_s2_timing_%i.pdf",runno));
  }
   ...
   ofstream oofile(Form("DB/LHRS_TOF_timing_%i.txt",runno));
  ...
  // use calculated corrections to plot new times with corrections
  ...
  ofstream oofile_csv(Form("DB/LHRS_TOF_timing_%i.csv",runno));
  ...
  TH2F* hsum_all = new TH2F("hsum_all","Corrected S2 Left + Right",16,0,15,200,2753-lr_off_mean,2780-lr_off_mean);

  hsum_all->GetXaxis()->SetTitle("TDC Channel (sum)"); hsum_all->GetXaxis()->CenterTitle();

  TH2F* hsum_all_l = new TH2F("hsum_all_l","corrected S2 Left",16,0,15,30,2780-left_off_mean,2810-left_off_mean);

  hsum_all_l->GetXaxis()->SetTitle("TDC Channel (sum)"); hsum_all_l->GetXaxis()->CenterTitle();

  TH2F* hsum_all_r = new TH2F("hsum_all_r","corrected S2 right",16,0,15,25,2725-right_off_mean,2750-right_off_mean);
  ...
  TH2F* hsum_all_un = new TH2F("hsum_all_un","Uncorrected S2 Left + Right",16,0,15,200,2753,2780);

  hsum_all_un->GetXaxis()->SetTitle("TDC Channel (sum)"); hsum_all_un->GetXaxis()->CenterTitle();

  TH2F* hsum_all_un_l = new TH2F("hsum_all_un_l","Uncorrected S2 Left",16,0,15,30,2780,2810);

  hsum_all_un_l->GetXaxis()->SetTitle("TDC Channel (sum)"); hsum_all_un_l->GetXaxis()->CenterTitle();

  TH2F* hsum_all_un_r = new TH2F("hsum_all_un_r","Uncorrected S2 right",16,0,15,25,2725,2750);
  ...
  for(Int_t i=0;i<nentries;i++){
	...
	Double_t left_time = L_s2_lt[j];
	Double_t right_time = L_s2_rt[j];
	Double_t lr_time = (left_time+right_time)/2.;

	Double_t left_time_corr = L_s2_lt[j] - left[j];
	Double_t right_time_corr = L_s2_rt[j] - right[j];
