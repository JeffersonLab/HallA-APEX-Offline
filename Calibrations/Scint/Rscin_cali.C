...
Script that performs pedestal and gain calibration for s2 for the RHRS.
...
void Rscin_cali(Int_t runno)
{
  ...
  ofstream myfile;
  myfile.open(Form("DB/Rscin_ped_%i.txt",runno));//,fstream::app);
  ...
  for(i=0;i<NS2Pad;i++){
    c1->cd(i+1);
    TH1F *tt1 = new TH1F("tt1",Form("FADC_Rs2_la%d",i+1),500,8000,12000);
    T->Draw(Form("R.s2.la[%d]>>tt1",i),tritype);
    tt1->SetXTitle(Form("R.s2.la[%d]",i));
  ...
  for(i=0;i<NS2Pad;i++){
    c2->cd(i+1);
    TH1F *tt2 = new TH1F("tt2",Form("Rs2_lap_%d",i+1),300,-200,1200);
    T->Draw(Form("R.s2.la_p[%d]>>tt2",i),tritype);
    tt2->SetXTitle(Form("R.s2.la_p[%d]",i));
  ...
  for(i=0;i<NS2Pad;i++){
    c3->cd(i+1);
    TH1F *tt3 = new TH1F("tt3",Form("FADC_Rs2_ra%d",i+1),500,8000,12000);
    T->Draw(Form("R.s2.ra[%d]>>tt3",i,i),tritype);
    tt3->SetXTitle(Form("R.s2.ra[%d]",i));
  ...
  for(i=0;i<NS2Pad;i++){
    c4->cd(i+1);
    TH1F *tt4 = new TH1F("tt4",Form("Rs2_rap_%d",i+1),300,-200,1200);
    T->Draw(Form("R.s2.ra_p[%d]>>tt4",i,i),tritype);
    tt4->SetXTitle(Form("R.s2.ra_p[%d]",i));
  ...
  // ped wid values (not in DB, written in DP-like format)
  myfile << endl;
  myfile << "R.s2.R.pedwid = " ;
  for(i=0;i<NS2Pad;i++){            
    myfile << Rped_width[i] << " ";
  }


  c1->Print(Form("plots/Rscin_%i.pdf[",runno));
  c1->Print(Form("plots/Rscin_%i.pdf",runno));
  c2->Print(Form("plots/Rscin_%i.pdf",runno));
  c3->Print(Form("plots/Rscin_%i.pdf",runno));
  c4->Print(Form("plots/Rscin_%i.pdf",runno));
  c4->Print(Form("plots/Rscin_%i.pdf]",runno));
  myfile.close();
