...
Performs S2 timing alignment for RHRS.
...
void R_time_alignment(Int_t runno){

  
  TChain* t = Load_more_rootfiles(runno);

  
  // cut idea R.s2.lt[5]>500&&R.s2.lt[4]>500&&R.s2.la_p[4]>200&&R.s2.la_p[5]>200

  const Int_t NS2Pad = 16;
  
  // Histograms for difference betwen adjacent paddle L-times and R-times
  // establish cuts for adjacent paddles also
  ...
  TString r_int_alt_cut[NS2Pad];

  

  Double_t L_ct_x_l[NS2Pad] = {-1.05,-0.84,-0.74,-0.58,-0.42,-1,-0.16,-0.03,0.1,0.24,0.38,0.53,0.66,0.78,0.93};
  Double_t L_ct_x_h[NS2Pad] = {-0.95,-0.78,-0.66,-0.45,-0.38,1,-0.-0.075,0.07,0.2,0.32,0.45,0.61,0.73,0.88,0.99};

  
  Double_t R_ct_x_l[NS2Pad] = {-1.03,-0.85,-0.73,-0.58,-0.43,-0.3,-0.13,-0.03,0.12,0.26,0.39,0.53,0.66,0.8,0.93};
  Double_t R_ct_x_h[NS2Pad] = {-0.87,-0.73,-0.63,-0.50,-0.35,-0.2,-0.08,0.07,0.22,0.34,0.45,0.6,0.74,0.9,1.01};
  ...
  for(int i=0;i<NS2Pad;i++){
    ...
    r_int_cut[i] = Form("R.s2.nthit==1 && R.s2.t_pads==%i && R.s2.la_c[%i]>100 && R.s2.ra_c[%i]>100 && abs(R.s2.trx)<1 && abs(R.s2.trdx)<0.07",i,i,i) + PID_cuts_r + GeneralSieveCut_r;


    r_int_alt_cut[i] = Form("R.s2.nthit==1 && R.s2.t_pads==%i && (R.s2.la_c[%i]>100 || R.s2.ra_c[%i]>100) && abs(R.s2.trx)<1 && abs(R.s2.trdx)<0.07 && R.tr.n==1",i,i,i);
  ...
  for(int i=0;i<NS2Pad-1;i++){
    ...
    h_adj_l[i] = new TH1F(adj_l_name[i],Form("R-Arm S2, L-paddles time difference %d and channel %d",i,i+1),40,-20,20);
    h_adj_l[i]->GetXaxis()->SetTitle(Form("(L paddle-%i) - (L paddle-%i), TDC channel",i,i+1));

    
    h_adj_r[i] = new TH1F(adj_r_name[i],Form("R-Arm S2, R-paddles time difference %d and channel %d",i,i+1),40,-20,20);
    h_adj_r[i]->GetXaxis()->SetTitle(Form("(R paddle-%i) - (R paddle-%i), TDC channel",i,i+1));

    h_adj_lr[i] = new TH1F(adj_lr_name[i],Form("R-Arm S2, L+R-paddles time difference %d and channel %d",i,i+1),40,-20,20);
    h_adj_lr[i]->GetXaxis()->SetTitle(Form("(L+R paddle-%i) - (L+R paddle-%i), TDC channel",i,i+1));

    l_adj_cut[i] = Form("R.s2.lt[%i]>500&&R.s2.la_p[%i]>100&&R.s2.lt[%i]>500&&R.s2.la_p[%i]>100",i,i,i+1,i+1);

    r_adj_cut[i] = Form("R.s2.rt[%i]>500&&R.s2.ra_p[%i]>100&&R.s2.rt[%i]>500&&R.s2.ra_p[%i]>100",i,i,i+1,i+1);

    lr_adj_cut[i] = Form("R.s2.lt[%i]>500&&R.s2.la_p[%i]>100&&R.s2.lt[%i]>500&&R.s2.la_p[%i]>100 && R.s2.rt[%i]>500&&R.s2.ra_p[%i]>100&&R.s2.rt[%i]>500&&R.s2.ra_p[%i]>100",i,i,i+1,i+1,i,i,i+1,i+1);

    
    l_ct_cut[i] = Form("L.s2.trx[%i]>%f&&L.s2.trx<%f",i,L_ct_x_l[i],L_ct_x_h[i]);
    //cout << l_ct_cut[i] << endl;
    //    r_ct_cut[i] = Form("R.s2.trx>%f&&R.s2.trx<%f",R_ct_x_l[i],R_ct_x_h[i]);
    r_ct_cut[i] = "abs(R.s2.trx)<1 && abs(R.s2.trdx)<0.07" + PID_cuts_r + GeneralSieveCut_r;
  ...
  for(int i=0;i<NS2Pad-1;i++){
    ...
    t->Draw(Form("R.s2.lt[%i]-R.s2.lt[%i]>>adj_hist_l_%i",i,i+1,i),l_adj_cut[i] + "&&" + r_ct_cut[i],"");
    ...
    t->Draw(Form("R.s2.rt[%i]-R.s2.rt[%i]>>adj_hist_r_%i",i,i+1,i),r_adj_cut[i] + "&&" + r_ct_cut[i],"");
    ...
    t->Draw(Form("((R.s2.lt[%i]+R.s2.rt[%i])-(R.s2.lt[%i]+R.s2.rt[%i]))>>adj_hist_lr_%i",i,i,i+1,i+1,i),lr_adj_cut[i] + "&&" + r_ct_cut[i],"");
    ...
    // cout << "Offsets between channel " << i << " and channel " << i+1 << "\t" << "l_offset = " << offset_l[i] << " (error = " << offset_l_err[i] << "), " << "\t" << "r_offset = " << offset_r[i] << " (error = " << offset_r_err[i] << "), " << "lr_offset = " << offset_lr[i] << " (error = " << offset_lr_err[i] << ") " << endl;
  ...
  c_l_adj->Print(Form("plots/s2_alignment/R_s2_l_alignment_%i.pdf",runno));
  
  c_r_adj->Print(Form("plots/s2_alignment/R_s2_r_alignment_%i.pdf",runno));
  
  c_lr_adj->Print(Form("plots/s2_alignment/R_s2_lr_alignment_%i.pdf",runno));
  
  



  /* Use difference in time between left and right pmts of same paddle with y position of hit in paddle (taken from vdc track projection) to get difference in timing offsets between left and right paddles
   */
  ...
  for(Int_t i = 0; i<NS2Pad; i++){
    ...
    //    t->Draw(Form("R.s2.lt[%i]-R.s2.rt[%i]:R.s2.try>>pdiff[%i]",i,i,i),Form("R.s2.t_pads[0]==%i",i),"prof");
    t->Draw(Form("R.s2.lt[%i]-R.s2.rt[%i]:R.s2.try>>pdiff[%i]",i,i,i),r_int_cut[i],"prof");
    ...
    if(pdiff[i]->GetFunction("pol1")){
      ...
      t->Draw(Form("R.s2.lt[%i]-R.s2.rt[%i]:R.s2.try>>pdiff[%i]",i,i,i),r_int_alt_cut[i],"prof");
  ...
  ofstream oofile(Form("DB/RHRS_S2_align_t_%i.txt",runno));

  oofile<<"RHRS Scintillator timing offsets: "<<endl<<endl;
  oofile<<"S2 Offsets: "<<endl;
  oofile<<"R.s2.L.off =  ";

  // minus sign is applied to corrections
  // this is as they are caluclated in time but applied as corrections
  // to TDC value which is in common stop (higher value is earlier time)
  ...
  ofstream oofile_csv(Form("DB/RHRS_S2_align_t_%i.csv",runno));
  ...
  for(int i=0;i<NS2Pad;i++){//(30,2530,2560)
      
    h_time_l[i] = new TH1F(Form("h_time_l_%i",i),Form("h_time_l_%i",i),30,2490-l_coeff[i],2520-l_coeff[i]);
    t->Draw(Form("R.s2.lt[%i]-%.9g >>h_time_l_%i",i,l_coeff[i],i),Form("R.s2.t_pads[0]==%i",i));    
    h_time_l[i]->Fit("gaus","RQ","",2490-l_coeff[i],2520-l_coeff[i]);
    if(h_time_l[i]->GetFunction("gaus")){
      l_mean_time[i] = h_time_l[i]->GetFunction("gaus")->GetParameter(1);
    }
    else{
      l_mean_time[i] = h_time_l[i]->GetMean();	
    }

    h_time_r[i] = new TH1F(Form("h_time_r_%i",i),Form("h_time_r_%i",i),25,2490-r_coeff[i],2520-r_coeff[i]);
    t->Draw(Form("R.s2.rt[%i]-%.9g >>h_time_r_%i",i,r_coeff[i],i),Form("R.s2.t_pads[0]==%i",i));


    h_time_r[i]->Fit("gaus","RQ","",2490-r_coeff[i],2520-r_coeff[i]);
    
    if(h_time_r[i]->GetFunction("gaus")){
      r_mean_time[i] = h_time_r[i]->GetFunction("gaus")->GetParameter(1);
    }
    else{
      r_mean_time[i] = h_time_r[i]->GetMean();	
    }

    //    r_mean_time[i] = h_time_r[i]->GetFunction("gaus")->GetParameter(1);

    h_time_lr[i] = new TH1F(Form("h_time_lr_%i",i),Form("h_time_lr_%i",i),27,2490-lr_coeff[i],2520-lr_coeff[i]);
    t->Draw(Form("((R.s2.lt[%i] - %.9g ) + (R.s2.rt[%i] - %.9g ))/2. >>h_time_lr_%i",i,l_coeff[i],i,r_coeff[i],i),Form("R.s2.t_pads[0]==%i",i));
    
    h_time_lr[i]->Fit("gaus","RQ","",2490-lr_coeff[i],2520-lr_coeff[i]);

    if(h_time_lr[i]->GetFunction("gaus")){
      lr_mean_time[i] = h_time_lr[i]->GetFunction("gaus")->GetParameter(1);
    }
    else{
      lr_mean_time[i] = h_time_lr[i]->GetMean();	
    }

    //    lr_mean_time[i] = h_time_lr[i]->GetFunction("gaus")->GetParameter(1);

    h_time_un_l[i] = new TH1F(Form("h_time_un_l_%i",i),Form("h_time_un_l_%i",i),30,2490,2520);
    t->Draw(Form("R.s2.lt[%i]>>h_time_un_l_%i",i,i),Form("R.s2.t_pads[0]==%i",i));    
    h_time_un_l[i]->Fit("gaus","RQ","",2490,2520);
    
    if(h_time_un_l[i]->GetFunction("gaus")){
      l_mean_time_un[i] = h_time_un_l[i]->GetFunction("gaus")->GetParameter(1);
    }
    else{
      l_mean_time_un[i] = h_time_un_l[i]->GetMean();	
    }


    h_time_un_r[i] = new TH1F(Form("h_time_un_r_%i",i),Form("h_time_un_r_%i",i),30,2490,2520);
    t->Draw(Form("R.s2.rt[%i]>>h_time_un_r_%i",i,i),Form("R.s2.t_pads[0]==%i",i));
    h_time_un_r[i]->Fit("gaus","RQ","",2490,2520);
    if(h_time_un_r[i]->GetFunction("gaus")){
      r_mean_time_un[i] = h_time_un_r[i]->GetFunction("gaus")->GetParameter(1);
    }
    else{
      r_mean_time_un[i] = h_time_un_r[i]->GetMean();	
    }

    //    r_mean_time_un[i] = h_time_un_r[i]->GetFunction("gaus")->GetParameter(1);

    h_time_un_lr[i] = new TH1F(Form("h_time_un_lr_%i",i),Form("h_time_un_lr_%i",i),30,2490,2520);
    t->Draw(Form("(R.s2.lt[%i] + R.s2.rt[%i])/2.>>h_time_un_lr_%i",i,i,i),Form("R.s2.t_pads[0]==%i",i));    
    h_time_un_lr[i]->Fit("gaus","RQ","",2490,2520);
    if(h_time_un_lr[i]->GetFunction("gaus")){
      lr_mean_time_un[i] = h_time_un_lr[i]->GetFunction("gaus")->GetParameter(1);
    }
    else{
      lr_mean_time_un[i] = h_time_un_lr[i]->GetMean();	
    }
