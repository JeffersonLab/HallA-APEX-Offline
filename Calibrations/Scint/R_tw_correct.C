...
void R_tw_correct(Int_t runno, Bool_t Coinc = true){
  ...
  TH1F* h_R_s0_t = new TH1F("h_R_s0_t","s0 left + right time",70,5.38e3,5.45e3);
  ...
  for(Int_t i = 0; i<NS2Pad; i++){
    ...
    h_R_s2_ADC_l[i] = new TH1F(Form("h_R_s2_ADC_l_%i",i),Form("s2 left TW effect, %i",i),tw_l_nbins,tw_l_low,tw_l_high);

    h_R_s2_ADC_r[i] = new TH1F(Form("h_R_s2_ADC_r_%i",i),Form("s2 right TW effect, %i",i),tw_r_nbins,tw_r_low,tw_r_high);


    // (left + right time)
    
    h_R_s2_t_ADC[i] = new TH2F(Form("h_R_s2_t_ADC_%i",i),Form("s2 left + right time vs (L+R) TW effect, %i",i),tw_b_nbins,tw_b_low,tw_b_high,tdiff_nbins,tdiff_l,tdiff_h);

    hprof_R_s2_t_ADC[i] = new TProfile(Form("hprof_R_s2_t_ADC_%i",i),Form("s2 left + right time vs (L+R) TW effect, %i",i),tw_b_nbins,tw_b_low,tw_b_high,tdiff_l,tdiff_h);

    h_R_s2_t_ADCl[i] = new TH2F(Form("h_R_s2_t_ADCl_%i",i),Form("s2 left + right time vs L TW effect, %i",i),tw_l_nbins,tw_l_low,tw_l_high,tdiff_nbins,tdiff_l,tdiff_h);

    hprof_R_s2_t_ADCl[i] = new TProfile(Form("hprof_R_s2_t_ADCl_%i",i),Form("s2 left + right time vs L TW effect, %i",i),tw_l_nbins,tw_l_low,tw_l_high,tdiff_l,tdiff_h);

    h_R_s2_t_ADCr[i] = new TH2F(Form("h_R_s2_t_ADCr_%i",i),Form("s2 left + right time vs R TW effect, %i",i),tw_r_nbins,tw_r_low,tw_r_high,tdiff_nbins,tdiff_l,tdiff_h);

    hprof_R_s2_t_ADCr[i] = new TProfile(Form("hprof_R_s2_t_ADCr_%i",i),Form("s2 left + right time vs R TW effect, %i",i),tw_r_nbins,tw_r_low,tw_r_high,tdiff_l,tdiff_h);


    // (left time)

    h_R_s2_tl_ADC[i] = new TH2F(Form("h_R_s2_tl_ADC_%i",i),Form("s2 left time vs (L+R) TW effect, %i",i),tw_b_nbins,tw_b_low,tw_b_high,tdiff_nbins,tdiff_l,tdiff_h);
	
    hprof_R_s2_tl_ADC[i] = new TProfile(Form("hprof_R_s2_tl_ADC_%i",i),Form("s2 left time vs (L+R) TW effect, %i",i),tw_b_nbins,tw_b_low,tw_b_high,tdiff_l,tdiff_h);
    
    h_R_s2_tl_ADCl[i] = new TH2F(Form("h_R_s2_tl_ADCl_%i",i),Form("s2 left time vs L TW effect, %i",i),tw_l_nbins,tw_l_low,tw_l_high,tdiff_nbins,tdiff_l,tdiff_h);

    hprof_R_s2_tl_ADCl[i] = new TProfile(Form("hprof_R_s2_tl_ADCl_%i",i),Form("s2 left time vs L TW effect, %i",i),tw_l_nbins,tw_l_low,tw_l_high,tdiff_l,tdiff_h);

    h_R_s2_tl_ADCr[i] = new TH2F(Form("h_R_s2_tl_ADCr_%i",i),Form("s2 left time vs R TW effect, %i",i),tw_r_nbins,tw_r_low,tw_r_high,tdiff_nbins,tdiff_l,tdiff_h);

    hprof_R_s2_tl_ADCr[i] = new TProfile(Form("hprof_R_s2_tl_ADCr_%i",i),Form("s2 left time vs R TW effect, %i",i),tw_r_nbins,tw_r_low,tw_r_high,tdiff_l,tdiff_h);


    // (right time)

    h_R_s2_tr_ADC[i] = new TH2F(Form("h_R_s2_tr_ADC_%i",i),Form("s2 right time vs (L+R) TW effect, %i",i),tw_b_nbins,tw_b_low,tw_b_high,tdiff_nbins,tdiff_l,tdiff_h);

    hprof_R_s2_tr_ADC[i] = new TProfile(Form("hprof_R_s2_tr_ADC_%i",i),Form("s2 right time vs (L+R) TW effect, %i",i),tw_b_nbins,tw_b_low,tw_b_high,tdiff_l,tdiff_h);

    h_R_s2_tr_ADCl[i] = new TH2F(Form("h_R_s2_tr_ADCl_%i",i),Form("s2 right time vs L TW effect, %i",i),tw_l_nbins,tw_l_low,tw_l_high,tdiff_nbins,tdiff_l,tdiff_h);

    hprof_R_s2_tr_ADCl[i] = new TProfile(Form("hprof_R_s2_tr_ADCl_%i",i),Form("s2 right time vs L TW effect, %i",i),tw_l_nbins,tw_l_low,tw_l_high,tdiff_l,tdiff_h);
    
    h_R_s2_tr_ADCr[i] = new TH2F(Form("h_R_s2_tr_ADCr_%i",i),Form("s2 right time vs R TW effect, %i",i),tw_r_nbins,tw_r_low,tw_r_high,tdiff_nbins,tdiff_l,tdiff_h);

    hprof_R_s2_tr_ADCr[i] = new TProfile(Form("hprof_R_s2_tr_ADCr_%i",i),Form("s2 right time vs R TW effect, %i",i),tw_r_nbins,tw_r_low,tw_r_high,tdiff_l,tdiff_h);
  ...
  for(Int_t i=0;i<nentries;i++){
    ...
    if(R_tr_n==1 && R_cer_asum_c>200 && (R_ps_asum_c + 0.9*R_sh_asum_c)>800 && R_s0_nthit==1 && CoincPass){
      h_R_s0_t->Fill(R_s0_lt[0] + R_s0_rt[0]);
      h_R_s0_ADC->Fill((1/TMath::Sqrt(R_s0_la_p[0])) + (1/TMath::Sqrt(R_s0_ra_p[0])));
      //      h_R_s0_t_ADC->Fill();
	...
	if (R_s2_lt[j]>0 && R_s2_rt[j]>0 && R_s2_la_c[j]>160 && R_s2_ra_c[j]>160 && R_s2_t_pads[0]==j){    
	  RHRS_pad = j;
	}	
      }
    }
    ...
    // condition on RHRS pad being found for single-arm run
    // or both LHRS and RHRS pad found for coincidence run
    if((Coinc && (LHRS_pad == -1 || RHRS_pad == -1)) || (!Coinc && (RHRS_pad == -1)) ){
      continue;
    }
    ...
    h_R_s2_t[RHRS_pad]->Fill(R_s2_lt[RHRS_pad] + R_s2_rt[RHRS_pad]);
    h_R_s2_ADC[RHRS_pad]->Fill((1/TMath::Sqrt(R_s2_la_p[RHRS_pad])) + (1/TMath::Sqrt(R_s2_ra_p[RHRS_pad])));

	  
    h_R_s2_t_l[RHRS_pad]->Fill(R_s2_lt[RHRS_pad]);
    h_R_s2_ADC_l[RHRS_pad]->Fill((1/TMath::Sqrt(R_s2_la_p[RHRS_pad])));


    h_R_s2_t_r[RHRS_pad]->Fill(R_s2_rt[RHRS_pad]);
    h_R_s2_ADC_r[RHRS_pad]->Fill((1/TMath::Sqrt(R_s2_ra_p[RHRS_pad])));
  ...
  for(Int_t i = 0; i<NS2Pad; i++){
    ...
    Double_t max_ADC = h_R_s2_ADC[i]->GetBinCenter(h_R_s2_ADC[i]->GetMaximumBin());
    Int_t max_entriess = h_R_s2_ADC[i]->GetBinContent(h_R_s2_ADC[i]->GetMaximumBin());
    ...
    Int_t l_check = h_R_s2_ADC[i]->GetBinContent(h_R_s2_ADC[i]->FindBin(s2_lr_lower));
    
    Int_t h_check = h_R_s2_ADC[i]->GetBinContent(h_R_s2_ADC[i]->FindBin(s2_lr_higher));
    ...
    Int_t l_check_l = h_R_s2_ADC_l[i]->GetBinContent(h_R_s2_ADC_l[i]->FindBin(s2_l_lower));
    
    Int_t h_check_l = h_R_s2_ADC_l[i]->GetBinContent(h_R_s2_ADC_l[i]->FindBin(s2_l_higher));
  ...
  for(Int_t i=0;i<nentries;i++){
    ...
    if(R_tr_n==1 && R_cer_asum_c>200 && (R_ps_asum_c + 0.9*R_sh_asum_c)>800 && R_s0_nthit==1 && CoincPass){      
      h_R_s0_t_ADC->Fill((1/TMath::Sqrt(R_s0_la_p[0])) + (1/TMath::Sqrt(R_s0_ra_p[0])), R_s0_lt[0] + R_s0_rt[0] - s0_t_mean);
      hprof_R_s0_t_ADC->Fill((1/TMath::Sqrt(R_s0_la_p[0])) + (1/TMath::Sqrt(R_s0_ra_p[0])), R_s0_lt[0] + R_s0_rt[0] - s0_t_mean);
    ...
    Double_t b_time =  R_s2_lt[RHRS_pad] + R_s2_rt[RHRS_pad] - s2_mean[RHRS_pad];
    Double_t l_time =  R_s2_lt[RHRS_pad] - s2_mean_l[RHRS_pad];
    Double_t r_time =  R_s2_rt[RHRS_pad] - s2_mean_r[RHRS_pad];
    
    
    // (left + right time)
    h_R_s2_t_ADC[RHRS_pad]->Fill(R_TW_p,b_time);
    hprof_R_s2_t_ADC[RHRS_pad]->Fill(R_TW_p,b_time);
    
    h_R_s2_t_ADCl[RHRS_pad]->Fill(R_TW_l,b_time);
    hprof_R_s2_t_ADCl[RHRS_pad]->Fill(R_TW_l,b_time);
    
    h_R_s2_t_ADCr[RHRS_pad]->Fill(R_TW_r,b_time);
    hprof_R_s2_t_ADCr[RHRS_pad]->Fill(R_TW_r,b_time);
    
	  

    // (left time)
    h_R_s2_tl_ADC[RHRS_pad]->Fill(R_TW_p,l_time);
    hprof_R_s2_tl_ADC[RHRS_pad]->Fill(R_TW_p,l_time);
	  
    h_R_s2_tl_ADCl[RHRS_pad]->Fill(R_TW_l,l_time);
    hprof_R_s2_tl_ADCl[RHRS_pad]->Fill(R_TW_l,l_time);

    h_R_s2_tl_ADCr[RHRS_pad]->Fill(R_TW_r,l_time);
    hprof_R_s2_tl_ADCr[RHRS_pad]->Fill(R_TW_r,l_time);


    // (right time)
    h_R_s2_tr_ADC[RHRS_pad]->Fill(R_TW_p,r_time);
    hprof_R_s2_tr_ADC[RHRS_pad]->Fill(R_TW_p,r_time);
		  
    h_R_s2_tr_ADCl[RHRS_pad]->Fill(R_TW_l,r_time);
    hprof_R_s2_tr_ADCl[RHRS_pad]->Fill(R_TW_l,r_time);
	  
    h_R_s2_tr_ADCr[RHRS_pad]->Fill(R_TW_r,r_time);
    hprof_R_s2_tr_ADCr[RHRS_pad]->Fill(R_TW_r,r_time);
  ...
  h_R_s0_t_ADC->Draw("colz");

  c2->cd(2);
  hprof_R_s0_t_ADC->Draw();
  ...
  if(!Coinc){
    ...
    s0_offset =  hprof_R_s0_t_ADC->GetFunction("pol1")->GetParameter(0);
    s0_k = hprof_R_s0_t_ADC->GetFunction("pol1")->GetParameter(1);
  ...
  for(Int_t i = 0; i<NS2Pad; i++){
    ...
    hprof_R_s2_t_ADC[i]->Draw();

    s2_ADC_l_cut_t[i]->SetLineColor(kMagenta);
    s2_ADC_h_cut_t[i]->SetLineColor(kMagenta);

    hprof_R_s2_t_ADC[i]->Fit("pol1","QR","",s2_ADC_l_val[i],s2_ADC_h_val[i]);
    
    if(hprof_R_s2_t_ADC[i]->GetFunction("pol1")){
    
      s2_offset[i] =  hprof_R_s2_t_ADC[i]->GetFunction("pol1")->GetParameter(0);
      s2_k[i] = hprof_R_s2_t_ADC[i]->GetFunction("pol1")->GetParameter(1);
    ...
    h_R_s2_t_ADCl[i]->Draw("colz");

    c4[i]->cd(4);

    hprof_R_s2_t_ADCl[i]->Draw();

    c4[i]->cd(5);

    h_R_s2_t_ADCr[i]->Draw("colz");

    c4[i]->cd(6);

    hprof_R_s2_t_ADCr[i]->Draw();
    ...
    h_R_s2_tl_ADCr[i]->Draw("colz");
    
    c4[i]->cd(9);
    
    h_R_s2_tl_ADCl[i]->Draw("colz");


    
    c4[i]->cd(10);
    
    hprof_R_s2_tl_ADCl[i]->Draw();
    ...
    //    hprof_R_s2_tl_ADCl[i]->Fit("pol1","QR","",s2_l_ADC_l_val[i],s2_l_ADC_h_val[i]);
    hprof_R_s2_tl_ADCl[i]->Fit("pol1","QR","",0.056,0.063);
    //hprof_R_s2_tl_ADCl[i]->Fit("pol1","QR","",0,0.16);

    if(hprof_R_s2_tl_ADCl[i]->GetFunction("pol1")){
    
      s2_offset_l[i] =  hprof_R_s2_tl_ADCl[i]->GetFunction("pol1")->GetParameter(0);
      s2_k_l[i] = hprof_R_s2_tl_ADCl[i]->GetFunction("pol1")->GetParameter(1);
    ...
    h_R_s2_tl_ADC[i]->Draw("colz");

    c4[i]->cd(8);

    hprof_R_s2_tl_ADC[i]->Draw();
    ...
    h_R_s2_tr_ADC[i]->Draw("colz");

    
    c4[i]->cd(14);
    
    hprof_R_s2_tr_ADC[i]->Draw();
    
    c4[i]->cd(15);

    h_R_s2_tr_ADCl[i]->Draw("colz");

    c4[i]->cd(16);

    hprof_R_s2_tr_ADCl[i]->Draw();

    c4[i]->cd(17);

    h_R_s2_tr_ADCr[i]->Draw("colz");

    c4[i]->cd(18);

    hprof_R_s2_tr_ADCr[i]->Draw();
