/**************************************************************
  ...
  This script fits the slopes of the real distance vs time plot for all four planes. This then provides the parameters for a modified version of the analytic method. 

  It's best run compiled

  analyzer
  .L AnalyticSlopeFit.C+
  AnalyticSlopeFit("L",runno)

  or

  AnalyticSlopeFit("R",runno)
...
Double_t Convert(Double_t dtime, const Double_t *par){


  //  const Double_t* CrossP = &(par[0]); // crossover point between regions 

  // first segment slope and intercept
  const Double_t* I1 = &(par[0]);
  const Double_t* M1 = &(par[1]);

  
  // second segment slope and intercept
  const Double_t* I2 = &(par[2]);
  const Double_t* M2 = &(par[3]);


  Double_t dist = 0.0;
  
  //  if(dtime > *CrossP){
  if(dtime > (*I2 - *I1)/(*M1 - *M2)){
    dist = *M1*dtime  + *I1;
  }
  else{
    dist = *M2*dtime  + *I2;
...
//Double_t TTDTable::ConvertAngleCorr(double dtime, double tanTheta){
Double_t TTDform_Alt(Double_t dtime, Double_t tanTheta, const Double_t *par){
  ...
  const Double_t* fR = &(par[5]); // radius of electric field of wire
  //  const Double_t* fslope0 = &(par[1]); // central angle
  const Double_t* fslope0 = &(par[6]); // central angle
  const Double_t fTheta0 = TMath::ATan(*fslope0);
  
  // tanTheta = TMath::ATan(1./tanTheta);
  ...
  // if (dtime >= (*CrossP)*SecTheta0 ) {
  //   dist += (*fR)*(SecTheta -  SecTheta0);
  // } else if (dtime < (*CrossP)*SecTheta0 ) { 
...
Double_t TTD_eval_Alt(Double_t *x, Double_t* par){

  Double_t dtime = x[0]/(1e9);
  
  Double_t* TTD_pars = &(par[0]);
  
  Double_t Distance  = TTDform_Alt(dtime, TT0, TTD_pars);
...
void fcn(Int_t& /*npar*/, Double_t* /*gin*/, Double_t& f, Double_t* par, Int_t /*iflag*/) {
  ...
  for (i=0; i<nent[this_plane]; i++) {
    ...
    // delta = TTDform_Alt(wtime[this_plane][i], 
    // 			1./tanTh[this_plane][i],
    // 			&par[0]) - trdist[this_plane][i];

    delta = Convert(wtime[this_plane][i],		        
			&par[0]) - trdist[this_plane][i];
...
void AnalyticSlopeFit( const char *arm = "L", Int_t runnumber = -1 ){
...
//     T->SetBranchAddress(Form("%s.vdc.%s.ltrdist", arm, plane[i]), hittrdist[i]);
  ...
  // plot difference between 'real' and TTD vs slope
  TH2F* hDistDiffSlope[NPLANE];
  
  // plot difference between 'real' and TTD vs slope (no angular correction)
  TH2F* hDistDiffSlopeUnCorr[NPLANE];
  ...
  Double_t finalvel[NPLANE], apars[NPLANE][7], dummy;
  ...
  for( i = 0; i < NPLANE; i++ ){
    ...
    hDistDiffSlope[i] = new TH2F(Form("Dist_diff_slope_%s", plane[i]),Form("Lookup table %s", plane[i]), 100, 1, 2, 100, -0.0015, 0.0015);
    hDistDiffSlope[i]->GetXaxis()->SetTitle("Slope");
    hDistDiffSlope[i]->GetYaxis()->SetTitle("Real distance - Table distance [m]");

    
    hDistDiffSlopeUnCorr[i] = new TH2F(Form("Dist_diff_slope_UnCorr_%s", plane[i]),Form("Lookup table (No Angular correction) %s", plane[i]), 100, 1, 2, 100, -0.0015, 0.0015);
    hDistDiffSlopeUnCorr[i]->GetXaxis()->SetTitle("Slope");
    hDistDiffSlopeUnCorr[i]->GetYaxis()->SetTitle("Real distance - Table distance [m]");
  ...
  for( i = 0; i < NPLANE; i++ ){
    ...
    //    gMinuit->mnparm(0, "CrossP",40e-9, 5e-10, 1e-10, 100e-9,ierflg); // CURRENT
    //    gMinuit->FixParameter(0);

    //    gMinuit->mnparm(1, "a1_0", 0.002, 1e-6, 0.001,0.003,ierflg);
    // gMinuit->mnparm(1, "I1",0, 1e-4, -5e-3,5e-3,ierflg);
    // gMinuit->mnparm(2, "M1",7.5e4, 1e2, 1e4,10e4,ierflg);

    // gMinuit->mnparm(3, "I2",0, 1e-4, -5e-3,5e-3,ierflg);
    // gMinuit->mnparm(4, "M2",5e4, 1e2, 1e4,10e4,ierflg);

    gMinuit->mnparm(0, "I1",0.00115689, 1e-4, -5e-3,5e-3,ierflg);    
    gMinuit->mnparm(1, "M1",49356.2, 1e2, 1e4,10e4,ierflg);

    gMinuit->mnparm(2, "I2",-0.00024284, 1e-4, -5e-3,5e-3,ierflg);
    gMinuit->mnparm(3, "M2",81072.2, 1e2, 1e4,10e4,ierflg);
    ...
    // CURRENT
    // gMinuit->mnparm(5, "R",0.002, 0.0001, 0.001, 0.005,ierflg);
    // gMinuit->mnparm(6, "theta0", 1.4, 0.001,1.0, 2.0,ierflg);
    ...
    */
    ...
    cout << plane[i] << ":" << endl;
    for( j = 0; j < 7; j++ ){
      gMinuit->GetParameter(j, apars[i][j], dummy );
      cout << "apars[" << i << "][" << j << "] = " << apars[i][j] << endl;
    }

    
    TTD_line[i] = new TF1(Form("TTD_eval_alt_%s",plane[i]),TTD_eval_Alt,-10,280, 11);
  ...
  for( i = 0; i < NPLANE; i++ ){
    ...
    for( j = 0; j < nent[i]; j++ ){      
      ...
      // hfit[i]->Fill( trdist[i][j], TTDform_Alt( wtime[i][j] - finaloff[i], tanTh[i][j], apars[i]) );
      // htime_dist[i]->Fill(1e9*(wtime[i][j] - finaloff[i]), TTDform_Alt( wtime[i][j] - finaloff[i], tanTh[i][j], apars[i]));

      hfit[i]->Fill( trdist[i][j], TTDform_Alt( wtime[i][j], 1./tanTh[i][j], apars[i]) );
      htime_dist[i]->Fill(1e9*(wtime[i][j]), TTDform_Alt( wtime[i][j], 1./tanTh[i][j], apars[i]));
      hDistDiffSlope[i]->Fill(1./tanTh[i][j],trdist[i][j] - TTDform_Alt( wtime[i][j], 1./tanTh[i][j], apars[i]));
      //      hDistDiffSlope[i]->Fill(Slope,RealDist-TTDDist);

      hDistDiffSlopeUnCorr[i]->Fill(1./tanTh[i][j],trdist[i][j] - Convert( wtime[i][j],  apars[i]));
  ...
  */
  ...
  TCanvas *c4 = new TCanvas(Form("Dist Diff v Slope (corrected) %s",plane[i]),Form("Dist Diff v Slope (corrected) %s",plane[i]), 640, 480);
  c4->Divide(2,2);
  ...
  TCanvas *c5 = new TCanvas(Form("Dist Diff v Slope %s",plane[i]),Form("Dist Diff v Slope %s",plane[i]), 640, 480);
  c5->Divide(2,2);

  for( i = 0; i < NPLANE; i++ ){
    c5->cd(i+1);
    hDistDiffSlopeUnCorr[i]->Draw("colz");
  }
  ...
  outp->open(Form("DB/ALT_analytic_TTD/db_%s_TTD.vdc.%d.dat", arm, runnumber) );
