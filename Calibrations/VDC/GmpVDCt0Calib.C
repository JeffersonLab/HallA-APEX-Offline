...
Double_t GmpVDCt0Calib::Findt0(const TH1* hnew) const
{
  ...
  gStyle->SetOptStat(0);
    ...
    Double_t t0 = 0.;
    cout << hnew->GetName() << "\t" << slope_min << "\t" << hnew->GetEntries() << endl;
    if (slope_min<-1 && hnew->GetEntries()>100.) {
      t0 = hnew->GetBinCenter(sdbin)-hnew->GetBinContent(sdbin)/slope_min;
      // t0 = hnew->GetBinCenter(sdbin)-(hnew->GetBinContent(sdbin) - hnew->GetBinContent(sdbin-1))/slope_min;
    }
    cout << "t0 == " << t0 << endl;
    if(t0 == 0.){
      cout << "Max bin is at t = " << hnew->GetBinCenter(maxbin) << endl;
    }
    else{
      //      cout << "t0 was not zero... :(" << endl;
    }
     ...
     // L.vdc.u1.tdc.offsets =
	  ...
	  if( fOffset[i][j] > 0){
	    *outfile  << setw(6) << fOffset[i][j] << " ";
	    ...
	    if(j/nWiresPerGroup > 0 &&  (fOffset[i,j-16]) != 0){
	      // case of not first group in plane and previous group offset is non-zero
	      *outfile  << setw(6) << fOffset[i][j-16] << " ";	      
	    }
	    else if(j/nWiresPerGroup < ((nWiresPerPlane/nWiresPerGroup)-1) && fOffset[i,j+16] != 0){
