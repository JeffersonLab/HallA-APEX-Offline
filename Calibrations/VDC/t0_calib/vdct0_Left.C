...
void vdct0_Left(int run) {
    ...
    //    const TString ROOTfilePath = "/chafs1/work1/tritium/rootfiles_1.6";
    //    const TString ROOTfilePath = "/chafs1/work1/apex/Rootfiles/";
    ...
    // str << ROOTfilePath << "apex_online_" << run;
    ...
    TString plotname = Form("plots/L_vdct0_%d.pdf",run);
    GmpVDCt0Calib* vdct0 = new GmpVDCt0Calib;

    Double_t tmin     = 1690, tmax = 1705;  // the limits for tg_y

    vdct0->Calibrate(T,1,"(DL.evtype==1)",plotname); 
    //    DL.evtype==6
    //    vdct0->Calibrate(T,1,"",plotname); // LHRS trigger
    vdct0->SaveDatabase(Form("DB/db_L_slope.vdc.%d.dat",run));
    ...
    for (UInt_t ii=0; ii<GmpVDCt0Calib::GetNumberOfPlanes(); ii++) {
        ...
        for (UInt_t j=0; j<GmpVDCt0Calib::GetNumberOfWiresPerPlane(); j++) {
	      ...
	      if(j/(vdct0->GetNumberOfWiresPerGroup()) > 0 && vdct0->GetOffset(ii,j-16) != 0){
		...
		graph->SetPoint(npoint++,j,offset);		
	      ...
	      else if( (j/vdct0->GetNumberOfWiresPerGroup()) <((GmpVDCt0Calib::GetNumberOfWiresPerPlane()/16)-1) && vdct0->GetOffset(ii,j+16) != 0){
    ...
    //    c1->SaveAs(Form("plot_L/vdct0plot_%d.C",run));
}
