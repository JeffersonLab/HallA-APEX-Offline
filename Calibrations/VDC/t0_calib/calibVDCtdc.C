...
void multirun_calibVDCtdc( const char *arm, int run = 4668) {
  ...
  //   T->Add( (*it).c_str() );
  // }
  // T->Add("/w/halla-2/e08014/yez/HRS_Cali/Right/replay/Rootfiles/e08014_R2_3590.root" );
  // T->Add("/w/halla-2/e08014/yez/HRS_Cali/Right/replay/Rootfiles/e08014_R2_3590_1.root" );
  // T->Add("/w/halla-2/e08014/yez/HRS_Cali/Right/replay/Rootfiles/e08014_R2_3590_2.root" );
  // T->Add("/w/halla-2/e08014/yez/HRS_Cali/Right/replay/Rootfiles/e08014_R2_3591.root" );
  // T->Add("/w/halla-2/e08014/yez/HRS_Cali/Right/replay/Rootfiles/e08014_R2_3591_1.root" );
  ...
  for( Int_t i = 0; i < NPLANE; i++ ){
    ...
    for( int grp = 0; grp < MAX_GRP; grp++ ) {
      //      h_tdc[i][grp] = new TH1I(Form("h_tdc_%s_%d",plane[i],grp+1), Form("%s TDC grp %d", plane[i], grp+1), 500, 900, 2000);
           h_tdc[i][grp] = new TH1I(Form("h_tdc_%s_%d",plane[i],grp+1), Form("%s TDC grp %d", plane[i], grp+1), 1000, 2000, 3000);
    }
  }
  ...
  Double_t sigma_est = 50, thresh_init = 0.9;
  ...
  //  outp->open(Form("DB/tdcoff_%s_%s.txt", arm, plane[i]), ios_base::out | ios_base::trunc );
  outp->open(Form("DB/db_%s_gaus.vdc.%d.dat", arm, run) );
  //  *outp<<arm<<".vdc."<<plane[i]<<".tdc.offsets="<<endl;
  cout << "Post-outp" << endl;
  ...
  for( Int_t i = 0; i < NPLANE; i++ ) {
	...
	if( TMath::Abs( xpk[k]-xmax ) < sigma_est ) {
	  ...
	  cout << endl;
	  cout << "xpk[" << k << "] = " << xpk[k] << ", xmax = " << xmax << ", simgma_est = " << sigma_est << endl;
	...
	line->SetLineWidth(3);
	line->Draw();
	ct->SaveAs("plots/t0-u1-grp7.pdf");
      }
   ...
   // Write data out in VDC database format
    ...
    cout<<arm<<".vdc."<<plane[i]<<".tdc.offsets="<<endl; 
    *outp<<arm<<".vdc."<<plane[i]<<".tdc.offsets="<<endl;
