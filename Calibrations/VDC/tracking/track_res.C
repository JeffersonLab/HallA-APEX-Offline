/**************************************************************
  ...
  Calculates sigma for timing offsets for clusters and resolution of local distance
...
void track_res(const char *arm = "L", Int_t runno = 4651, const char* title = ""){
  ...
  TCut AngCut = XAngCut + YAngCut + Form("%s.vdc.u1.slope != %s.vdc.u1.lslope && %s.vdc.v1.slope != %s.vdc.v1.lslope && %s.vdc.u2.slope != %s.vdc.u2.lslope && %s.vdc.v2.slope != %s.vdc.v2.lslope", arm, arm, arm, arm, arm, arm, arm, arm);
  ...
  for(Int_t i = 0; i<NAngle; i++){
    ...
    T->Draw(Form("%s.vdc.%s.lslope-%s.vdc.%s.slope>>hmuDiff_%i",arm,plane[2*i],arm,plane[2*i],i),AngCut,"0");
    ...
    T->Draw(Form("%s.vdc.%s.lslope-%s.vdc.%s.slope>>hmvDiff_%i",arm,plane[2*i+1],arm,plane[2*i+1],i),AngCut,"0");
  ...
  TLegend *legChi2 =  new TLegend(.50,.65,.9,.9,"Key");
  legChi2->SetFillColor(0);
  legChi2->SetTextSize(0.025);
  legChi2->AddEntry(fChi2,"ideal #chi^{2} with 7 dof","l");
  legChi2->Draw();
