...
void hole_id(){
  ...
  //for(int n_row = 0; n_row < 17; n_row++){
      ...
      tcuts->GetObject(Form("e_hcut_L_%d_%d_%d",FoilID,n_col,n_row), g);
	...
	x_col = n_col;
      ...
      TEllipse* Ellipse = new TEllipse(idx_00-x_col*dx,idy_00-n_row*dy,0.02,0.02);
      ...
      tcuts->GetObject(Form("FPhcut_L_%d_%d_%d",FoilID,n_col,n_row), g_FP);
