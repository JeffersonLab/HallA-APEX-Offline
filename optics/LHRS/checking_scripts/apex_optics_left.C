...
void apex_optics_left(Int_t run_no = 4179){
  ...
  GeneralCut = cut_nclust + cut_ntrack; // + TriggerCut
  GeneralCut += ct1+ct2;


  TH1F* hep_ratio = new TH1F("hep_ratio","hep_ratio",200,0,1.5); 
  hep_ratio->SetTitle("Shower energy to momentum ratio");

  TCanvas *c5 = new TCanvas("c5","LHRS Shower plots",1000,800); 

  c5->cd(1);

  T->Draw("(L.prl1.e+L.prl2.e)/L.tr.p",GeneralCut);
  ...
  c5->Print("apex_optics_left5.pdf");
