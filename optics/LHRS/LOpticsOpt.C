...
// 			   dphi/fNRawData,TMath::Sqrt(rmsphi/fNRawData));
	...
	if (PrintEachHole)
	{
		const Double_t PrintLimit = 4e-4; //rad
		DEBUG_INFO("SumSquareDTh","Print Deviation and RMS of each hole (|Dev| > %f):",PrintLimit);
...
//		DEBUG_INFO("SumSquareDTh","Average |deviation| = %f +/- %f (stat. w.)"
//				,weighted_dev/weight,TMath::Sqrt(1/weight));
		DEBUG_INFO("SumSquareDTh","Average |deviation| = %f "
				,sum_absdev/sum_cnt);
...
//				                         );
		...
		DEBUG_INFO("SumSquareDTh",tmp.Data());
...
// 	DEBUG_INFO("PrepareSieve","Entry Point");
	...
	for(UInt_t idx = 0; idx<NFiols; idx++){
	  ...
	  Real_angles[idx]  = new TH2D("Real_angles","Real_angles",500,-0.04,0.04,500,-0.08,0.08);
	}
...
TCanvas * LOpticsOpt::CheckVertex()
{
	...
	for(UInt_t FoilID = 0; FoilID<NFiols; FoilID++){
	  ...
	  h_dvz_rms[FoilID] = new TH2D(Form("dVz rms all holes Foil_%d",FoilID),Form("dVz rms all holes Foil_%d",FoilID),27,0,26,17,0,16);
	...
	for(UInt_t FoilID = 0; FoilID<NFiols; FoilID++){
	  hYVSrealTgY[FoilID] = new TH2D(Form("hYVSrealTgY_%d",FoilID),Form("Rot. Y VS 'real' tg_y (Foil_%d)",FoilID),1000,-0.1,0.1, 1000,-.1,.1);

	  hYVScalcTgY[FoilID] = new TH2D(Form("hYVScalcTgY_%d",FoilID),Form("Rot. Y VS 'calc' tg_y (Foil_%d)",FoilID),1000,-0.1,0.1, 1000,-.1,.1);
...
TCanvas * LOpticsOpt::CheckVertexRes()
{
	//Visualize ReactZ spectrum
  ...
  DEBUG_INFO("CheckVertex","Entry Point");
	...
	for(UInt_t FoilID = 0; FoilID<2; FoilID++){
...
TCanvas * LOpticsOpt::CheckVertexDiff()
{
  ...
  TH2D * hrbxVSReactZ = new TH2D("hrbxVSReactZ","Beam X VS ReactZ",1000,-VertexRange,VertexRange, 1000,-.001,.003);
