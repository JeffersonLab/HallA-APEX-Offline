...
static const char* types[] = 
  {"","Char_t","Short_t","Int_t","Long_t","Float_t",
   "Int_t","","Double_t","","","UChar_t","UShort_t",
...
public:
  ...
  DataElement( const char* s, const char* f=::format ) : 
    TNamed(s,s), good(false), write(false), data(0), size(0), leafcount(0), 
    branch(0), min(0), max(1), fmt(f) {}
...
Int_t Init()
{
  ...
  if( fChain == 0 ) {
    cerr << "Tree "<<treename<<" not found. Can't do a thing." 
	 << endl;
    return 1;
  }
  fCurrent = -1;
...
Int_t WriteVariables(FILE* fi, int icut)
{
  ...
  while((item = (DataElement*)next()) && retval == 0) {
    if( !item->write || !item->good ) continue;
...
Int_t LoadDefinedVariables()
{
  ...
  while( i<len && sscanf(p+i,"%s%n",buf,&l)>0 ) {
    ...
    retval = 2;
    if( lbrk != kNPOS ) {
      if( rbrk == kNPOS || rbrk <= lbrk+1 )  break;
      range = s(lbrk+1,rbrk-lbrk-1);
    } else if( rbrk != kNPOS )
      break;
    if( col != kNPOS ) {
      if( rbrk != kNPOS && col <= rbrk ) break;
	...
	if( elem->min >= elem->max )
	  break;
	if( elem->IsArray() && !elem->IsVarArray() &&
	    elem->max > elem->size )
	  break;
  ...
  case 2:
    ...
    break;
  case 3:
    cerr << "Index out of range for variable " << name 
	 << " (definition: " << buf << ") " << endl;
    break;
  default:
    break;
...
// 	bre = 0;
    ...
    while( *typ && strcmp(*typ,leaf->GetTypeName())) typ++;
    if( *typ && **typ ) {
...
Int_t LoadGCuts()
{
  ...
  while(( key=(TKey*)next() )) {
	...
	//Added to scale cuts back to rad instead of mrad
...
Int_t LoadPeakDefinitions()
{
 // Read definitions of cuts for the peaks.
  ...
  while( fgets(buf, BUFLEN, fi) ) {
    ...
    if( pos != kNPOS ) tbuf.Remove(pos);
    char* cbuf = Compress(tbuf.Data());
    TString sbuf(cbuf);
    delete [] cbuf;
    if(sbuf.IsNull()||sbuf[0]=='#') continue;
    if(lDebug)
      cout << "Cut " << npk << ": " << tbuf << endl;
...
void usage()
{
  if( !program || !*program ) program="tree2ascii";
...
int main( int argc, char** argv )
{
  ...
  // Parse command line
  getargs(argc,argv);
  if( lDefFile && LoadVarDefFile()) return 1;
  if( fVarDef.IsNull()) {
    if( lDebug )
  ...
  while((rootfile = (TObjString*)next())) {
    ...
    if( lDebug ) 
      cout << "Tree reports " << nentries << " events. "
	   << "Event limits: read: " << nmax << " write: " << nout << endl;
