...
ROpticsOpt * opt;
UInt_t NPara = 0;
Double_t OldMatrixArray1[5000] = {-99}; 
Double_t OldMatrixArray2[5000] = {-99}; 
Double_t OldMatrixArray[10000] = {-99}; //NPara
Bool_t freed1[5000] = {kFALSE}; 
Bool_t freed2[5000] = {kFALSE}; 
...
void DoMinTP(TString SourceDataBase1, TString SourceDataBase2, TString DestDataBase, UInt_t MaxDataPerGroup = 200)
{
    // minimize with root
    ...
    opt->LoadDataBase(SourceDataBase1,1);
    opt->LoadDataBase(SourceDataBase2,2);
    
    int NPara1 = opt->Matrix2Array(OldMatrixArray1, (*opt->fCurrentMatrixElems),freed1);
    int NPara2 = opt->Matrix2Array(OldMatrixArray2, (*opt->fCurrentMatrixElems2),freed2);
   
    NPara = NPara1 + NPara2;
    for(int i = 0; i<NPara;i++) {
      if(i<NPara1) {
	OldMatrixArray[i] = OldMatrixArray1[i];
	freed[i] = freed1[i];
      }
      if(i>=NPara1) {
	OldMatrixArray[i] = OldMatrixArray2[i];
	freed[i] = freed2[i]; 
      }
    }
    ...
    opt->SaveDataBase(DestDataBase + "_MHRS",1);
    opt->SaveDataBase(DestDataBase + "_Ms",2); 
...
void DoMinY(TString SourceDataBase, TString DestDataBase, UInt_t MaxDataPerGroup = 200)
{
    ...
    opt->LoadDataBase(SourceDataBase,1);
    ...
    opt->Print("");
    opt->SaveDataBase(DestDataBase,1);
...
void PlotDataBase(TString DatabaseFileName, UInt_t MaxDataPerGroup = 1000)
{
    ...
    opt->LoadDataBase(DatabaseFileName,1);
    opt->Print("");
...
void ROpticsOptScript(TString select, TString SourceDataBase1, TString SourceDataBase2, TString DestDataBase)
{
    opt = new ROpticsOpt();

    DestDataBase = "DB/" + DestDataBase;
    SourceDataBase1 = "DB/" + SourceDataBase1;
    SourceDataBase2 = "DB/" + SourceDataBase2;
    TString SourceDataBase = SourceDataBase1;
    ...
    case 1:
        ...
        DoMinTP(SourceDataBase1,SourceDataBase2, DestDataBase, 500);
        break;
    case 2:
        cout << "Optimizing for Phi\n";
        myfcn = myfcn2;
        opt->fCurrentMatrixElems = &(opt->fPMatrixElems);
	opt->fCurrentMatrixElems2 = &(opt->fPMatrixElems2);
	DoMinTP(SourceDataBase1,SourceDataBase2, DestDataBase, 500);
        break;
    case 3:
