...
Int_t ROpticsOpt::LoadDataBase(TString DataBaseName, int DataBaseNum)
{
    ...
    fTMatrixElems2.clear();
    fDMatrixElems2.clear();
    fPMatrixElems2.clear();
    fPTAMatrixElems2.clear();
    fYMatrixElems2.clear();
    fYTAMatrixElems2.clear();
    fRMatrixElems2.clear();

    fFPMatrixElems2.clear();
    fFPMatrixElems2.resize(3);
    ...
    matrix_map["t"] = &fFPMatrixElems2;
    matrix_map["y"] = &fFPMatrixElems2;
    matrix_map["p"] = &fFPMatrixElems2;
    matrix_map["D"] = &fDMatrixElems2;
    matrix_map["T"] = &fTMatrixElems2;
    matrix_map["Y"] = &fYMatrixElems2;
    matrix_map["YTA"] = &fYTAMatrixElems2;
    matrix_map["P"] = &fPMatrixElems2;
    matrix_map["PTA"] = &fPTAMatrixElems2;
    matrix_map["R"] = &fRMatrixElems2;
    ...
    CalcMatrix(1., fRMatrixElems2); // tensor without explicit polynomial in x_fp
...
Int_t ROpticsOpt::SaveDataBase(TString DataBaseName, int DataBaseNum)
{
    ...
    if(DataBaseNum ==2){
    DEBUG_INFO("SaveDataBase", "Transport Matrix:  D-terms");
    for (vsiz_t i = 0; i < fDMatrixElems2.size(); i++) {
        fprintf(file, "D ");
        const THaMatrixElement& m = fDMatrixElems2[i];
        for (vsiz_t j = 0; j < m.pw.size(); j++) {
            fprintf(file, "%d ", m.pw[j]);
        }
        int j;
    ...
    DEBUG_INFO("SaveDataBase", "Transport Matrix:  T-terms");
    for (vsiz_t i = 0; i < fTMatrixElems2.size(); i++) {
        fprintf(file, "T ");
        const THaMatrixElement& m = fTMatrixElems2[i];
        for (vsiz_t j = 0; j < m.pw.size(); j++) {
            fprintf(file, "%d ", m.pw[j]);
        }
        int j;
    ...
    DEBUG_INFO("SaveDataBase", "Transport Matrix:  P-terms");
    for (vsiz_t i = 0; i < fPMatrixElems2.size(); i++) {
        fprintf(file, "P ");
        const THaMatrixElement& m = fPMatrixElems2[i];
        for (vsiz_t j = 0; j < m.pw.size(); j++) {
            fprintf(file, "%d ", m.pw[j]);
        }
        int j;
    ...
    DEBUG_INFO("SaveDataBase", "Transport Matrix:  Y-terms");
    for (vsiz_t i = 0; i < fYMatrixElems2.size(); i++) {
        fprintf(file, "Y ");
        const THaMatrixElement& m = fYMatrixElems2[i];
        for (vsiz_t j = 0; j < m.pw.size(); j++) {
            fprintf(file, "%d ", m.pw[j]);
        }
        int j;
     ...
     DEBUG_INFO("SaveDataBase", "Transport Matrix:  PTA-terms");
    for (vsiz_t i = 0; i < fPTAMatrixElems2.size(); i++) {
        fprintf(file, "PTA ");
        const THaMatrixElement& m = fPTAMatrixElems2[i];
        for (vsiz_t j = 0; j < m.pw.size(); j++) {
            fprintf(file, "%d ", m.pw[j]);
        }
        int j;
...
void ROpticsOpt::check_fit_qual_y()
{
...
Double_t ROpticsOpt::SumSquareXpos()
{
    // return square sum of diff between calculated tg_th and expected tg_th
    ...
    Double_t theta_fwd;  //theta from forward matrix
    Double_t theta_back; //theta from backward matrix
	...
	const TVector3 BeamSpotHCS(eventdata.Data[kBeamX], eventdata.Data[kBeamY], targetfoils[FoilID]);
	const TVector3 BeamSpotTCS = fTCSInHCS.Inverse()*(BeamSpotHCS - fPointingOffset);
	const Double_t x_tg = BeamSpotTCS.X() - BeamSpotTCS.Z() * tan(eventdata.Data[kRealTh]);


	
        // calculate the matrices we need
	CalcMatrix(x_fp, fTMatrixElems);
	CalcMatrix(x_tg, fTMatrixElems2);
        

        // calculate the coordinates at the target
        theta_back = CalcTargetVar(fTMatrixElems, powers);
	theta_fwd = CalcTargetVar(fTMatrixElems2, powers);
	
	
        dth += theta_back -  theta_fwd;
        rmsth += dth*dth;

        DEBUG_MASSINFO("SumSquareDTh", "D_Th = %f = \t%f - \t%f", theta - eventdata.Data[kRealThMatrix], theta, eventdata.Data[kRealThMatrix]);

        // save the results
        eventdata.Data[kCalcTh] = theta_fwd;
    }
...
Double_t ROpticsOpt::SumSquareYpos()
{
    // return square sum of diff between calculated tg_ph and expected tg_ph
	...
	const Double_t x_tg = BeamSpotTCS.X() - BeamSpotTCS.Z() * tan(eventdata.Data[kRealTh]);
        ...
        // calculate the matrices we need
	CalcMatrix(x_fp, fPMatrixElems);
	CalcMatrix(x_tg, fPMatrixElems2);
        

        // calculate the coordinates at the target
        phi_back = CalcTargetVar(fPMatrixElems, powers);
	phi_fwd = CalcTargetVar(fPMatrixElems2, powers);

	
        dphi += phi_back - phi_fwd;
        rmsphi += dphi*dphi;

        DEBUG_MASSINFO("SumSquareDPhi", "D_Phi = %f = \t%f - \t%f", phi - eventdata.Data[kRealPhi], phi, eventdata.Data[kRealPhi]);

        //save the results
        eventdata.Data[kCalcPh] = phi_fwd;
