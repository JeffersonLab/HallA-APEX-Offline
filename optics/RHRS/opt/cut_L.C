...
int Run_number = 4647;
TString Range = "-10_10";
TString WorkDir = Form("../Sieve/%i_test/xfp_"+Range+"/",Run_number);
...
//string RootFile_opt1 = "apex_4653_opt_5th_xfp_full_V_Opt1.root";
...
TString get_Beamcut(Int_t runnumber){
  ...
  if(runnumber == 4648){
    ...
    beam_cut = "0.00115<Rrb.x && Rrb.x<0.0024";
    //    beam_cut = "0.00175<Lrb.x && Lrb.x<0.0021";
  }
  else if(runnumber == 4647){
    // V2
    cout << "V2 wire beam cut!" << endl;
    beam_cut = "-0.0016<Rrb.x && Rrb.x<-0.0004";
    // beam_cut = "-0.0008<Lrb.x && Lrb.x<-0.0004";
  }
  else if(runnumber == 4650){
    // V3
    cout << "V3 wire beam cut!" << endl;
    beam_cut = "-0.0028<Rrb.x && Rrb.x<-0.0014";
...
void which_foil(int &startFoil, int &nFoil){

  if(Run_number == 4648) {
    startFoil = 0;
    nFoil = 1;
  }
  else if(Run_number == 4647) {
    startFoil = 1;
    nFoil = 1;
  }
  else if(Run_number == 4650) {
    startFoil = 2;
    nFoil = 1;
  }
  else if(Run_number == 4653) {
    startFoil = 3;
    nFoil = 4;
  }
  else if(Run_number == 4652) {
    startFoil = 7;
    nFoil = 4;
  }
...
void cut_Vertex() {
  ...
  cout << "cut being used " << GeneralCut << endl << endl;
  ...
  TH2F* h1 = new TH2F("h1", Form("Vertex for Foil #%d;\\phi_{tg} [rad],Reactz [m]",FoilID), 400, phlowlimit*1000, phuplimit*1000, 400, vzlowlimit*1000, vzuplimit*1000);
  ...
  cout << "GeneralCut = " << GeneralCut << endl;
  //T->Draw("reactz:ph_tgt>>h1", GeneralCut, "COLZ"); // need finer delta cut later
  ...
  //	T->Draw("L.tr.r_ph:L.tr.r_y>>h1", GeneralCut, "COLZ"); // need finer delta cut later


  c1->Update();

  // Choose the foil you want to make cut
  int startFoil = 1;    //3 for Optics 1 and 7 for Optics 3
  int nFoil = 1;
  
  which_foil(startFoil,nFoils);

  int endFoil = startFoil + nFoils;
  
    for (int FoilID = startFoil; FoilID < endFoil; FoilID++) {
        TCut DrawCut = GeneralCut;

	T->Draw("R.tr.vz*1000:R.tr.tg_ph*1000>>h1", DrawCut, "COLZ");
	c1->Update();
        ...
        SaveCanvas(c1, WorkDir + RootFileName + Form(".fcut_R_%d", FoilID), kFALSE);
    }
...
void cut_Vertex_FP_SF(int overwrite = 0, int nfoils = 3, int FoilID = -1, int append = 0) {
  ...
  T->Draw("L.tr.r_ph:L.tr.r_y>>h2", GeneralCut + Form("fcut_L_%d", FoilID), "COLZ"); // need finer delta cut later
  ...
  cutdesc <<  Form("fcut_L_%d", i) << " && " <<  Form("fcut_L_FP_%d", i) << " && " << (const char *) GeneralCut << endl;
  }
...
void cut_Vertex_FP_MF(int overwrite = 0, int nfoils = 3, int FoilID = -1, int append = 0) {
  ...
  T->Draw(Form("L.tr.r_ph:L.tr.r_y>>h_FP_comp_%d",0), GeneralCut + TCut(Form("fcut_L_%d", 0+offset)) +  TCut(Form("!fcut_L_%d", 2+offset)) + TCut(Form("!fcut_L_%d", 4+offset)) + TCut(Form("!fcut_L_%d", 6+offset)) ,  "");



  h_FP_comp[1]->SetMarkerColor(kMagenta+3);

  //  T->Draw(Form("L.tr.r_ph:L.tr.r_y>>h_FP_comp_%d",0), GeneralCut + TCut(Form("!fcut_L_%d", 0+offset)) +  TCut(Form("fcut_L_%d", 2+offset)),  "SCAT");


  //  cout << "cut 2 = " << Form("!fcut_L_%d", 0+offset) + Form("fcut_L_%d", 2+offset) << endl;

  T->Draw(Form("L.tr.r_ph:L.tr.r_y>>h_FP_comp_%d",1), GeneralCut + TCut(Form("fcut_L_%d", 0+offset)) +  TCut(Form("fcut_L_%d", 2+offset)) + TCut(Form("!fcut_L_%d", 4+offset)) + TCut(Form("!fcut_L_%d", 6+offset)),  "SCAT SAME");


  h_FP_comp[2]->SetMarkerColor(kRed);


  T->Draw(Form("L.tr.r_ph:L.tr.r_y>>h_FP_comp_%d",2), GeneralCut + TCut(Form("!fcut_L_%d", 0+offset)) +  TCut(Form("fcut_L_%d", 2+offset)) + TCut(Form("!fcut_L_%d", 4+offset)) + TCut(Form("!fcut_L_%d", 6+offset)),  "SCAT SAME");
  ...
  T->Draw(Form("L.tr.r_ph:L.tr.r_y>>h_FP_comp_%d",3), GeneralCut + TCut(Form("!fcut_L_%d", 0+offset)) +  TCut(Form("fcut_L_%d", 2+offset)) + TCut(Form("fcut_L_%d", 4+offset)) + TCut(Form("!fcut_L_%d", 6+offset)),  "SCAT SAME");
  ...
  T->Draw(Form("L.tr.r_ph:L.tr.r_y>>h_FP_comp_%d",4), GeneralCut + TCut(Form("!fcut_L_%d", 0+offset)) +  TCut(Form("!fcut_L_%d", 2+offset)) + TCut(Form("fcut_L_%d", 4+offset)) + TCut(Form("!fcut_L_%d", 6+offset)),  "SCAT SAME");
  ...
  T->Draw(Form("L.tr.r_ph:L.tr.r_y>>h_FP_comp_%d",5), GeneralCut + TCut(Form("!fcut_L_%d", 0+offset)) +  TCut(Form("!fcut_L_%d", 2+offset)) + TCut(Form("fcut_L_%d", 4+offset)) + TCut(Form("fcut_L_%d", 6+offset)),  "SCAT SAME");
  ...
  T->Draw(Form("L.tr.r_ph:L.tr.r_y>>h_FP_comp_%d",6), GeneralCut + TCut(Form("!fcut_L_%d", 0+offset)) +  TCut(Form("!fcut_L_%d", 2+offset)) + TCut(Form("!fcut_L_%d", 4+offset)) + TCut(Form("fcut_L_%d", 6+offset)),  "SCAT SAME");
  ...
  for (int i = fmin; i < nfoil + fmin; i++) {
    ...
    //    T->Draw("L.tr.r_ph:L.tr.r_y>>h2", GeneralCut + TCut(Form("fcut_L_%d",i)), "COLZ"); // need finer delta cut later


    T->Draw("L.tr.r_ph:L.tr.r_y:th_tgt>>h2", GeneralCut + TCut(Form("fcut_L_%d",i)), "COLZ"); 
...
void display_Foils(){
 ...
 T->Draw("L.tr.r_ph:L.tr.r_y>>h2", GeneralCut, "COLZ"); 
...
void display_Sieve(int FoilID = 0, int col = 0){
 ...
 cout << "General cut = " << GeneralCut << endl << endl;
 ...
 // T->Draw("L.tr.tg_th:L.tr.tg_ph>>h2", GeneralCut + TCut(Form("fcut_L_%d", FoilID)), "COLZ");
  



 T->Draw("(1000*th_tgt):(1000*ph_tgt)>>h2", GeneralCut + TCut(Form("fcut_L_%d", FoilID)) + TCut(Form("fcut_L_FP_%d", FoilID)), "COLZ");



  // test of how many events pass each cut
  cout << "How many pass overall events: " << T->GetEntries() << endl;
  cout << "How many pass general cut: " << T->GetEntries(GeneralCut) << endl;
  ...
  cout << "How many pass all cuts: " << T->GetEntries(GeneralCut + TCut(Form("fcut_L_FP_%d", FoilID)) + TCut(Form("fcut_L_%d", FoilID))) << endl;
 ...
 //  TH2F* h2 = new TH2F("h2", "theta_target vs. phi_target", 300, -0.06, 0.04, 300, -0.08, 0.08);
  ...
  T->Draw("x_sieve:y_sieve_alt>>h3", GeneralCut + TCut(Form("fcut_L_FP_%d", FoilID)) + TCut(Form("fcut_L_%d", FoilID)), "COLZ");
  ...
  T->Draw("x_sieve:y_sieve>>h3_b", GeneralCut + TCut(Form("fcut_L_FP_%d", FoilID)) + TCut(Form("fcut_L_%d", FoilID)), "COLZ");
 ...
 //  TH2F* h2 = new TH2F("h2", "theta_target vs. phi_target", 300, -0.06, 0.04, 300, -0.08, 0.08);
  ...
  T->Draw("1000*th_tgt:1000*ph_tgt>>h2_cp", GeneralCut + TCut(Form("fcut_L_%d", FoilID)) + TCut(Form("fcut_L_FP_%d", FoilID)), "COLZ");
 ...
 //  TH2F* h2 = new TH2F("h2", "theta_target vs. phi_target", 300, -0.06, 0.04, 300, -0.08, 0.08);
  ...
  T->Draw("1000*th_tgt:1000*ph_tgt>>h2", GeneralCut + TCut(Form("fcut_L_FP_%d", FoilID)) + TCut(Form("fcut_L_%d", FoilID)), "COLZ");
 ...
 //  TH2F* h2 = new TH2F("h2", "theta_target vs. phi_target", 300, -0.06, 0.04, 300, -0.08, 0.08);
  ...
  T->Draw("(1000*th_tgt):(L.tr.r_y)>>h6", GeneralCut + TCut(Form("fcut_L_FP_%d", FoilID)) + TCut(Form("fcut_L_%d", FoilID)), "COLZ");
 ...
 //  TH2F* h2 = new TH2F("h2", "theta_target vs. phi_target", 300, -0.06, 0.04, 300, -0.08, 0.08);
  ...
  c6->cd(1);
  //  T->Draw("(L.tr.r_th):(L.tr.r_y)>>h7_b", GeneralCut + TCut(Form("fcut_L_FP_%d", FoilID)) + TCut(Form("fcut_L_%d", FoilID)), "COLZ");
  T->Draw("1000*th_tgt:1000*ph_tgt>>h2", GeneralCut + TCut(Form("fcut_L_FP_%d", FoilID)) + TCut(Form("fcut_L_%d", FoilID)), "COLZ");
  c6->cd(2);
  T->Draw("(1000*th_tgt):(L.tr.r_y)>>h6", GeneralCut + TCut(Form("fcut_L_FP_%d", FoilID)) + TCut(Form("fcut_L_%d", FoilID)), "COLZ");
...
void display_all_Sieve(TString plot_name = ""){
  ...
  for(UInt_t i = 0; i<(sizeof(foil_nos)/sizeof(foil_nos[0])); i++ ){
    ...
    T->Draw(Form("(1000*th_tgt):(1000*ph_tgt)>>h2_%d",FoilID), GeneralCut + TCut(Form("fcut_L_%d", FoilID)) + TCut(Form("fcut_L_FP_%d", FoilID)) + "mycut", "COLZ");
    ...
    // cout << "How many pass foil cut: " << T->GetEntries(TCut(Form("fcut_L_%d", FoilID))) << endl;
    // cout << "How many pass foil FP cut: " << T->GetEntries(TCut(Form("fcut_L_FP_%d", FoilID))) << endl;
    // cout << "How many pass all cuts: " << T->GetEntries(GeneralCut + TCut(Form("fcut_L_FP_%d", FoilID)) + TCut(Form("fcut_L_%d", FoilID))) << endl << endl;
    ...
    T->Draw(Form("L.tr.r_th*1000:L.tr.r_y>>thfp_v_yfp_%d",FoilID),GeneralCut + TCut(Form("fcut_L_%d", FoilID)) + TCut(Form("fcut_L_FP_%d", FoilID)) + "mycut","colz");
  ...
  T->Draw("reactz:ph_tgt>>h_f_tg", GeneralCut + "mycut", "COLZ");
  ...
  T->Draw("L.tr.r_ph:L.tr.r_y>>h1", GeneralCut + "mycut", "COLZ");
...
void CutSieve_ellipse(int FoilID = 0, uint col = 0, int overwrite = 0, int append = 1){
  ...
  T->Draw("th_tgt:ph_tgt>>h2", GeneralCut + Form("fcut_L_%d", FoilID) + TCut(Form("fcut_L_FP_%d", FoilID)) , "COLZ");
  ...
  for (int cnt = 0; cnt < nhol; cnt++) {
   ...
   c1->cd(0);
    ...
    cutdesc << Form("e_hcut_L_%d_%d_%d", FoilID, col, k) << " && " << Form("fcut_L_%d", FoilID) << " && " << Form("fcut_L_FP_%d", FoilID) << " && " << (const char *) GeneralCut << endl;
  }
...
void CutSieve_FP(int FoilID = 0, uint col = 0, int overwrite = 0, int append = 1){
  ...
  // John W: attempt to add secondary cut on FP theta vs y. This acts on a secondary check on accidentaly including events wrongly identified from a different hole (or background).
    ...
    T->Draw("L.tr.r_th*1000:L.tr.r_y>>thfp_v_yfp",GeneralCut + TCut(Form("fcut_L_%d", FoilID)) + TCut(Form("fcut_L_FP_%d", FoilID)),"colz");
	...
	continue;
      ...
      T->Draw("L.tr.r_th*1000:L.tr.r_y>>thfp_v_yfp",GeneralCut + TCut(Form("fcut_L_%d", FoilID)) + TCut(Form("fcut_L_FP_%d", FoilID)) + TCut(Form("e_hcut_L_%d_%d_%d", FoilID, col, k)),"colz");
	...
	cout << "reached end of loop" << endl;
      ...
      cutdesc << Form("FPhcut_L_%d_%d_%d",FoilID, col, k) << " && " << Form("e_hcut_L_%d_%d_%d", FoilID, col, k) << " && " << Form("fcut_L_%d", FoilID) << " && " << Form("fcut_L_FP_%d", FoilID) << " && " << (const char *) GeneralCut << endl;
...
void Optics_3_th_ph() {
  ...
  for(Int_t i = 0; i<4; i++){
    ...
    //    T->Draw(Form("th_tgt:ph_tgt>>h2_%d",i), GeneralCut + TCut(Form("fcut_L_%d", foil_no)) + TCut(Form("fcut_L_FP_%d", foil_no)), "COLZ");
    //    T->Draw(Form("th_tgt:ph_tgt>>h2_%d",i), GeneralCut + TCut(Form("fcut_L_FP_%d", foil_no)), "COLZ");


    // new optimisation
    T->Draw(Form("th_tgt:ph_tgt>>h2_%d",i), GeneralCut + TCut(Form("fcut_L_%d", foil_no)), "COLZ");

    // old optimisation
    //    T->Draw(Form("L.tr.tg_th:L.tr.tg_ph>>h2_%d,i"), GeneralCut + TCut(Form("fcut_L_%d",foil_no)), "COLZ");
    ...
    //    T->Draw(Form("x_sieve:y_sieve_alt>>h3_%d",i), GeneralCut + TCut(Form("fcut_L_%d", foil_no)) + TCut(Form("fcut_L_FP_%d", foil_no)), "COLZ");

    T->Draw(Form("x_sieve:y_sieve_alt>>h3_%d",i), GeneralCut + TCut(Form("fcut_L_%d", foil_no)), "COLZ");


    // T->Draw(Form("x_sieve:y_sieve_alt>>h3_%d",i), GeneralCut + TCut(Form("fcut_L_FP_%d", foil_no)), "COLZ");
    ...
    T->Draw(Form("L.tr.r_ph:L.tr.r_y>>h_FP_%d",i), GeneralCut + TCut(Form("fcut_L_%d", foil_no)), "COLZ");
    // T->Draw(Form("L.tr.r_ph:L.tr.r_y>>h_FP_%d",i), GeneralCut + TCut(Form("fcut_L_FP_%d", foil_no)), "COLZ");


    c_FP_theta[i]->cd(0);
    
    
    T->Draw(Form("L.tr.r_ph:L.tr.r_y:th_tgt>>h_FP_theta_%d",i), GeneralCut + TCut(Form("fcut_L_%d", foil_no)), "COLZ");
  ...
  T->Draw("reactz:ph_tgt>>h_Foil_comp", GeneralCut, "COLZ"); 
...
void CutSieve(int i = 0 /*Foil ID*/, int cmin = 0 /*Column ID*/, int overwrite = 0, bool append = true) {
  ...
  T->Draw("L.tr.tg_th:L.tr.tg_ph>>h2", GeneralCut
  ...
  for (int j = cmin; j < ncol + cmin; j++) //column number j
	...
	cutdesc << Form("fcut_L_%d && hhcut_L_%d_%d_%d", i, i, j, k) << " && " << (const char *) GeneralCut
		<< endl;
      }
    }
...
//	gSystem->Exec("cp -vf ./cutfiles/gcut_L_dpfull.root ./cutfiles/gcut_L_dpfull.root.old2");
	...
	for(UInt_t FoilID = 0;FoilID<10;FoilID++)
	{
		...
		t->Draw(Form("L.tr.tg_th:L.tr.tg_dp>>hDpCutFoil%d(1000,-.01,.05,1000,-.1,.1)",FoilID)
				,TCut(VertexCut) && GeneralCut,"COLZ");
		c1->Update();
