...
TCanvas * LOpticsOpt::CheckSieve(Int_t PlotFoilID)
{
    ...
    cout << " fine at this junction" << endl;
	...
	PlotID -= 8;
      ...
      */
      ...
      const TVector3 BeamSpotHCS_average(BeamX_average[FoilID],BeamY_average[FoilID],targetfoils[FoilID]);
      ...
      HSieveAngle[PlotID]->Fill(eventdata.Data[kCalcPh],eventdata.Data[kCalcTh]);

      cout<<idx<<" "<<FoilID<<" "<<eventdata.Data[kCalcPh]<<" "<<eventdata.Data[kCalcTh]<<endl;
	...
	*/
	...
	for (UInt_t Col = 0; Col < NSieveCol; Col++) {
	  ...
	  TVector3 BeamSpotHCS_average(BeamX_average[PlotID],BeamY_average[PlotID],targetfoils[PlotID]);
...
TCanvas * LOpticsOpt::Sieve_hole_diff(Int_t PlotFoilID, TString select){
  ...
  // create plots for each foil of difference between expected and real sieve x and y 
   ...
   Double_t dX[NFoils] = {0}, dY[NFoils] = {0};
    ...
    if(select == "theta") return c1;
    if(select == "phi") return c2;
...
TCanvas * LOpticsOpt::Sieve_hole_diff_tail(Int_t PlotFoilID,TString select){
...
TCanvas * LOpticsOpt::CheckVertex_Surv(){

  // script designed to plot and compare survey y_tg and reactz calculated from this
